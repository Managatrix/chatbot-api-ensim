			...
			public void run() {
				...
				if (response != null && !response.getResult().isEmpty()) {
					...
					if ((words.size() >= 2 && words.get(1).equalsIgnoreCase("joke"))
						...
						// if (words.get(0).equalsIgnoreCase("add")) {
						// // Demande de création de blague
						// MessageRestController messageRestController = new MessageRestController();
						// messageRestController.sendMessage("You have requested to add a question/answer joke to the database. Please provide a question.");

						// JokeRestController jokeRestController = new JokeRestController();
						// jokeRestController.creerEquipe(jokeToAdd);
						// }
					...
					if (words.get(0).equalsIgnoreCase("meteo")) {
						...
						if (words.size() == 1) {
							meteoResponseMessage = "You need to provide a city name.\nExample : Meteo Marseille";
						} else {
					...
					if (words.get(0).equalsIgnoreCase("help")) {
						...
						URI messageURL = UriComponentsBuilder.fromUriString("http://127.0.0.1:9090")
								.path("/message")
								.queryParam("text", "Try these commands :\n\t- (Bad or Good) Joke\n\t- Meteo [City]\n")
								.build().encode().toUri();
						restTemplate.getForObject(messageURL, ApiResponseTelegram.class);
					}

					// If unknown command
					if (!words.get(0).equalsIgnoreCase("help") && !words.get(0).equalsIgnoreCase("meteo")
							&& !words.get(0).equalsIgnoreCase("bad") && !words.get(0).equalsIgnoreCase("good")
							&& !words.get(0).equalsIgnoreCase("funny") && !words.get(0).equalsIgnoreCase("joke")) {
						// sendMessage(text) request
						URI messageURL = UriComponentsBuilder.fromUriString("http://127.0.0.1:9090")
								.path("/message")
